...
[]{.question options="1"} 
Proposer un montage expérimental qui n’induise que de la flexion (démontrer en calculant le torseur global en $x=L$ que le montage proposé n’induit que de la flexion). Mêmes questions pour la torsion.


:::{.solutionbox arguments="15cm"}

- Pour n’imposer que de la flexion, il suffit tout simplement de suspendre un poids au bout de la barre :

![$R(\tau) = -F \underline{j}$](figures/solutions/torsion/flexion.png){width="4cm" fig-align="center"}


- Pour n’imposer que de la torsion, il suffit par exemple d’imposer deux forces opposées orientées selon j qui induiront une résultante nulle et un moment selon i

![$R(\tau) = -F \underline{j}$](figures/solutions/torsion/torsion.png){width="4cm" fig-align="center"}
...
Déterminer les expressions théoriques des déformations aux points 2 et 3 en fonction de $E$, $\nu$, $R$, $l_0$, $l_1$ et $F$. Puis, faites l’application numérique en vous fixant une longueur de bras de levier $l_0$ comprise entre $50$ et $450~mm$ et une masse comprise entre $5$ et $15~kg$. Déterminer alors toutes les déformations et les directions principales associées au point 3. 


::: {.solutionbox arguments="10cm"}

- Pour déterminer les déformations aux 4 points particuliers, il suffit d’appliquer la loi de Hooke (matériau élastique linéaire):
$$
\underline{\underline{E}} = \dfrac{1+\nu}{E}  \underline{\underline{\Sigma}} - \dfrac{\nu}{E}\text{trace}(\underline{\underline{\Sigma}}) \underline{\underline{I}} 
$$

à partir des expressions théoriques des matrices des contraintes en ces points, on obtient ainsi :
...
Rappeler le principe d’une jauge de déformation. Rappeler le principe d’un pont de Wheastone et son intérêt. Quels sont les avantages du montage $\frac{1}{2}$ pont sur le montage  $\frac{1}{4}$ de pont ?

::: {.solutionbox arguments="3cm"}
...
[]{.question options="1"} 
Justifier l’utilisation d’une rosette triaxiale au point 2. Même question pour la rosette bi-axiale au point 3.
...
Déterminer, pour les points 2 et 3, les relations entre les informations expérimentales fournies par les différentes jauges et les composantes de la matrice des déformations en ces points. 
...
\question Etude de la linéarité du comportement au point 2 de la jauge orientée selon $\underline{a}$. En vous appuyant sur les résultats des questions 3.5. et 3.2., quelle est la relation théorique exacte attendue entre la réponse en déformation  mesurée   et la charge appliquée F ? 
...
- Etude du point 2 en $\frac{1}{4}$ de pont.
...
\begin{mytextbox}[colupper=blue,fontupper=\bfseries, height=10cm ]
- Etude du point 2 en $\dfrac{1}{2}$ pont
